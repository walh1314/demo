...
import { getCameraLamp } from "../../http/camera";
...
export default {
  ...
  methods: {
    ...
    mouseup(event) {
      ...
      // this.sourceX = x;
      //     this.sourceY = y;
      const juliX = this.sourceX - x;
      const juliY = this.sourceY - y;
      for (var i = 0; i < _lamps.length; i++) {
        if (this.sourceI == i) {
          points = _lamps[i].points;
          for (var j = 0; j < points.length; j++) {
      ...
      for (var j = 0; j < form.points.length; j++) {
        if (this.addPonitsJ == j) {
          ...
          this.addPonitsJ = -1;
        } else if (form.points[j].isSelect) {
    ...
    initDraw() {
      var _this = this;
      getCameraLamp({ deviceId: this.deviceId })
        .then(res => {
