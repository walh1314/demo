...
public class EncryptUtil
{
	 ...
	 * DES算法密钥
	 */
	private static final byte[] USE_TOKEN_DES_KEY = { 11, 13, -10, 92, -32, -83, -118, -55 };
	 ...
	 * @return 加密后的数据
	 */
	public static String encryptUserTokenDes(String data) {
		String encryptedData = null;
		try {
			// DES算法要求有一个可信任的随机数源
			SecureRandom sr = new SecureRandom();
			DESKeySpec deskey = new DESKeySpec(USE_TOKEN_DES_KEY);
			// 创建一个密匙工厂，然后用它把DESKeySpec转换成一个SecretKey对象
			SecretKeyFactory keyFactory = SecretKeyFactory.getInstance("DES");
			SecretKey key = keyFactory.generateSecret(deskey);
			// 加密对象
	 ...
	 * @return 解密后的数据
	 */
	public static String decryptUserTokenDes(String cryptData) {
		String decryptedData = null;
		try {
			// DES算法要求有一个可信任的随机数源
			SecureRandom sr = new SecureRandom();
